...
# Author:   Jakub Wlodek
# Created:  12-Aug-2019
# Docs:     https://jwlodek.github.io/py_tui-docs
# License:  BSD-3-Clause (New/Revised)
#

# py_tui uses the curses library. On Windows this does not exist, but
# there is an open source windows-curses module that adds curses support
# for python on Windows
import time

import fastlogging  # Use logging library for debug purposes
...
# py_tui imports
import py_tui.keys
import py_tui.statusbar
import py_tui.widgets
import py_tui.controls
import py_tui.dialogs
import py_tui.widget_set
import py_tui.popups
import py_tui.renderer
import py_tui.debug
import py_tui.errors
from py_tui.colors import *
...
class PyCUI:
    ...
    Main user interface class for py_tui. To create a user interface, you must
    ...
    grid : py_tui.grid.Grid
        The main layout manager for the CUI
    widgets : dict of str - py_tui.widgets.Widget
        dict of widget in the grid
    title_bar : py_tui.statusbar.StatusBar
        a status bar object that gets drawn at the top of the CUI
    status_bar : py_tui.statusbar.StatusBar
        a status bar object that gets drawn at the bottom of the CUI
    keybindings : list of py_tui.keybinding.KeyBinding
    ...
    def __init__(
            ...
            exit_key=py_tui.keys.KEY_Q_LOWER,
        ...
        self.title_bar = py_tui.statusbar.StatusBar(
            self._title, BLACK_ON_WHITE, root=self, is_title_bar=True
        )
        exit_key_char = py_tui.keys.get_char_from_ascii(exit_key)
        ...
        self.status_bar = py_tui.statusbar.StatusBar(
        ...
        # Logging object initialization for py_tui
        # noinspection PyProtectedMember
        self._logger = py_tui.debug._initialize_logger(self, name="py_tui")

        # Initialize grid, renderer, and widget dict
        self.grid = py_tui.grid.Grid(
        ...
        self._widgets: Dict[int, Optional["py_tui.widgets.Widget"]] = {}
        self._renderer: Optional["py_tui.renderer.Renderer"] = None
        ...
        self._forward_cycle_key = py_tui.keys.KEY_CTRL_LEFT
        self._reverse_cycle_key = py_tui.keys.KEY_CTRL_RIGHT
    ...
    def enable_logging(
            self,
            log_file_path: str = "py_tui.log",
            logging_level=fastlogging.DEBUG,
            live_debug_key: int = py_tui.keys.KEY_CTRL_D,
    ) -> None:
        """Function enables logging for py_tui library

        Parameters
        ----------
        live_debug_key
        log_file_path : str
            The target log filepath. Default 'py_tui_log.txt
        logging_level : int
    ...
    def apply_widget_set(self, new_widget_set: py_tui.widget_set.WidgetSet) -> None:
        """Function that replaces all widgets in a py_tui with those of a different widget set
        ...
        if isinstance(new_widget_set, py_tui.widget_set.WidgetSet):
            self.lose_focus()
            self._widgets = new_widget_set.widgets
            self.grid = new_widget_set._grid
            self._keybindings = new_widget_set.keybindings
            ...
            raise TypeError("Argument must be of type py_tui.widget_set.WidgetSet")

    def create_new_widget_set(
            self, num_rows: int, num_cols: int
    ) -> "py_tui.widget_set.WidgetSet":
    ...
    def _initialize_colors(self) -> None:
        ...
        # noinspection PyProtectedMember
        for color_pair in py_tui.colors._COLOR_MAP.keys():
            # noinspection PyProtectedMember
            fg_color, bg_color = py_tui.colors._COLOR_MAP[color_pair]
            curses.init_pair(color_pair, fg_color, bg_color)

        for key in py_tui.colors.USER_COLOR_PAIRS.keys():
            # noinspection PyProtectedMember
            fg_color, bg_color = key
            curses.init_pair(py_tui.colors.USER_COLOR_PAIRS[key], fg_color, bg_color)

    @property
    def get_extended_colour(self) -> bool:
    ...
    def _initialize_widget_renderer(self) -> None:
        ...
        if self._renderer is None:
            self._renderer = py_tui.renderer.Renderer(self, self._stdscr, self._logger)
        for widget_id in self.get_widgets().keys():
            ...
            if widget is not None:
                ...
                except py_tui.errors.PyCUIError:
                    self._logger.debug(
    ...
    def get_widgets(self) -> Dict[int, Optional["py_tui.widgets.Widget"]]:
    ...
    def add_custom_widget(self, widget_class: type, title: str, row: int, column: int, row_span: int, column_span: int,
                          padx: int, pady: int, *args, **kwargs) -> 'py_cui.widgets.Widget':
        ...
        if not issubclass(widget_class, py_tui.widgets.Widget):
            raise TypeError(f'Widget class {widget_class} is not a subclass of the base Widget class!')

        id = len(self.get_widgets().keys())
        new_widget = widget_class(id, title, self._grid, row, column, row_span, column_span, padx, pady, self._logger,
                                  *args, **kwargs)
    ...
    def add_scroll_menu(
        ...
        return self.add_custom_widget(py_tui.widgets.ScrollMenu,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_checkbox_menu(
        ...
        return self.add_custom_widget(py_tui.widgets.CheckBoxMenu,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_text_box(
        ...
        return self.add_custom_widget(py_tui.widgets.TextBox,
                                      title,
                                      row, column,
                                      row_span,
                                      column_span,
    ...
    def add_text_block(
            ...
            pady: int = 0,
            initial_text: str = "",
    ) -> "py_tui.widgets.ScrollTextBlock":
        ...
        return self.add_custom_widget(py_tui.widgets.ScrollTextBlock,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_label(
        ...
        return self.add_custom_widget(py_tui.widgets.Label,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_block_label(
        ...
        return self.add_custom_widget(py_tui.widgets.BlockLabel,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_button(
        ...
        return self.add_custom_widget(py_tui.widgets.Button,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def add_slider(
            ...
            step: int = 1,
            init_val: int = 0,
    ) -> "py_tui.controls.slider.SliderWidget":
        """Function that adds a new label to the CUI grid
        ...
        return self.add_custom_widget(py_tui.controls.slider.SliderWidget,
                                      title,
                                      row,
                                      column,
                                      row_span,
    ...
    def forget_widget(self, widget: "py_tui.widgets.Widget") -> None:
        """Function that is used to destroy or "forget" widgets. Forgotten widgets will no longer be drawn

        Parameters
        ----------
        widget : py_tui.widgets.Widget
            Widget to remove from the UI

        Raises
        ------
        TypeError
            If input parameter is not of the py_tui widget type
        KeyError
            If input widget does not exist in the current UI or has already been removed.
        """

        if not isinstance(widget, py_tui.widgets.Widget):
            raise TypeError("Argument widget must by of type py_tui.widgets.Widget!")
    ...
    def _get_horizontal_neighbors(
            self, widget: "py_tui.widgets.Widget", direction: int
    ) -> Optional[List[int]]:
        ...
        if not direction in py_tui.keys.ARROW_KEYS:
            return None

        _, num_cols = self.grid.get_dimensions()
        row_start, col_start = widget.get_grid_cell()
        row_span, col_span = widget.get_grid_cell_spans()
        id_list = []

        if direction == py_tui.keys.KEY_LEFT_ARROW:
            col_range_start = 0
    ...
    def _get_vertical_neighbors(
        ...
        if direction not in (py_tui.keys.KEY_UP_ARROW, py_tui.keys.KEY_DOWN_ARROW):
            return None

        num_rows, _ = self.grid.get_dimensions()
        row_start, col_start = widget.get_grid_cell()
        row_span, _ = widget.get_grid_cell_spans()
        id_list = []

        if direction == py_tui.keys.KEY_UP_ARROW:
        ...
        for row in scope:
            row_items = [
                w for w in ids if widgets[w].row == row and widgets[w].is_selectable()
            ]
            sorted_items = sorted(
                row_items, key=lambda x: abs(col_start - widgets[x].column)
            )
    ...
    def _check_if_neighbor_exists(self, direction: int) -> Optional[int]:
        ...
        if self._selected_widget is not None:
            start_widget: Optional[py_tui.widgets.Widget] = self.get_widgets()[
        ...
        if start_widget is not None:
            if direction in [py_tui.keys.KEY_DOWN_ARROW, py_tui.keys.KEY_UP_ARROW]:
                neighbors = self._get_vertical_neighbors(start_widget, direction)
            elif direction in [py_tui.keys.KEY_RIGHT_ARROW, py_tui.keys.KEY_LEFT_ARROW]:
    ...
    def get_selected_widget(self) -> Optional["py_tui.widgets.Widget"]:
        """Function that gets currently selected widget

        Returns
        -------
        selected_widget : py_tui.widgets.Widget
    ...
    def lose_focus(self) -> None:
        """Function that forces py_tui out of focus mode.
    ...
    def move_focus(
            self, widget: "py_tui.widgets.Widget", auto_press_buttons: bool = True
    ) -> None:
        ...
        if (
                ...
                and isinstance(widget, py_tui.widgets.Button)
        ):
        ...
        elif self._auto_focus_buttons and isinstance(widget, py_tui.widgets.Button):
            self.status_bar.set_text(self._init_status_bar_text)
        else:
    ...
    def _cycle_widgets(self, reverse: bool = False) -> None:
        ...
        if (
                ...
                and cycle_key in current_widget.key_commands.keys()
        ):
            return
    ...
    def show_message_popup(
        ...
        self._popup = py_tui.popups.MessagePopup(
    ...
    def show_yes_no_popup(self, title: str, command: Callable[[bool], Any]):
        ...
        self._popup = py_tui.popups.YesNoPopup(
            self,
            title + "- (y/n)",
            "Yes - (y), No - (n)",
            color,
    ...
    def show_text_box_popup(
        ...
        color = WHITE_ON_BLACK
        self._popup = py_tui.popups.TextBoxPopup(
    ...
    def show_menu_popup(
        ...
        color = WHITE_ON_BLACK
        self._popup = py_tui.popups.MenuPopup(
            self,
            menu_items,
            title,
            color,
    ...
    def show_loading_icon_popup(
        ...
        color = WHITE_ON_BLACK
        self._loading = True
        self._popup = py_tui.popups.LoadingIconPopup(
    ...
    def show_loading_bar_popup(
        ...
        color = WHITE_ON_BLACK
        self._loading = True
        self._popup = py_tui.popups.LoadingBarPopup(
    ...
    def show_form_popup(
        ...
        self._popup = py_tui.dialogs.form.FormPopup(
            self,
            fields,
            passwd_fields,
            required,
    ...
    def show_filedialog_popup(
        ...
        self._popup = py_tui.dialogs.filedialog.FileDialogPopup(
            self,
            callback,
            initial_dir,
            popup_type,
    ...
    def _refresh_height_width(self) -> None:
        ...
        self._height = height
        self._width = width
        self.grid.update_grid_height_width(self._height, self._width)
        for widget_id in self.get_widgets().keys():
    ...
    def _handle_key_presses(self, key_pressed: int) -> None:
        ...
        # for the escape key, which exits focus mode.
            ...
            if key_pressed == py_tui.keys.KEY_ESCAPE:
                self.status_bar.set_text(self._init_status_bar_text)
                self._in_focused_mode = False
                ...
                # widget handles remaining py_tui.keys
                self._logger.debug(
        ...
        # Otherwise, barring a popup, we are in overview mode, meaning that arrow py_tui.keys move between widgets,
        # and Enter key starts focus mode
        elif self._popup is None:
            if (
                    key_pressed == py_tui.keys.KEY_ENTER
            ...
            # If not in focus mode, use the arrow py_tui.keys to move around the selectable widgets.
            neighbor = None
            if key_pressed in py_tui.keys.ARROW_KEYS:
        ...
        while (
            ...
            try:
                ...
                if key_pressed == curses.KEY_RESIZE:
                    try:
                        self._refresh_height_width()
                    except py_tui.errors.PyCUIOutOfBoundsError as e:
                ...
                # Here we handle mouse click events globally, or pass them to the UI element to handle
                    ...
                    if valid_mouse_event:
                        ...
                        # Otherwise, if not a popup, select the clicked on widget
                                ...
                                in_element, py_tui.popups.Popup
                        ):
