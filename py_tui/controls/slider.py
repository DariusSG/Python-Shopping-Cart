...
class SliderImplementation(py_tui.ui.UIImplementation):
    ...
    def __init__(self, min_val: int, max_val: int, init_val: int, step: int, logger):
        ...
        if self._cur_val < self._min_val or self._cur_val > self._max_val:
            raise py_tui.errors.PyCUIInvalidValue(
...
class SliderWidget(py_tui.widgets.Widget, SliderImplementation):
    """
    Widget for a Slider
    Parameters
    ----------
    ...
    def __init__(self, id, title, grid, row, column, row_span, column_span,
        ...
        py_tui.widgets.Widget.__init__(self, id, title, grid, row, column,
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if key_pressed == py_tui.keys.KEY_RIGHT_ARROW:
            self.update_slider_value(1)
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event: int):
        ...
        if mouse_event == py_tui.keys.LEFT_MOUSE_CLICK or mouse_event == py_tui.keys.LEFT_MOUSE_RELEASED:
...
class SliderPopup(py_tui.popups.Popup, SliderImplementation):
    pass