...
The grid is currently the only supported layout manager for py_tui
"""
...
class Grid:
    ...
    def __init__(self, num_rows: int, num_columns: int, height: int, width: int, logger: 'py_tui.debug.PyCUILogger'):
    ...
    def set_num_rows(self, num_rows: int) -> None:
        ...
        if (3 * num_rows) >= self._height:
            raise py_tui.errors.PyCUIOutOfBoundsError
        self._num_rows = num_rows
        self._row_height = int(self._height / self._num_rows)
    ...
    def update_grid_height_width(self, height: int, width: int):
        ...
        self._offset_x = self._width % self._num_columns - 1
        self._offset_y = self._height % self._num_rows - 1
        self._logger.debug(f'Updated grid. Cell dims: {self._row_height}x{self._column_width}, Offsets {self._offset_x},{self._offset_y}')
