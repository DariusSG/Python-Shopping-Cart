"""Module containing all core widget classes for py_tui.

Widgets are the basic building blocks of a user interface made with py_tui.
...
from py_tui.constants import Colors

from typing import Union, Callable, List, Dict, Tuple, Any, Optional


class Widget(py_tui.ui.UIElement):
    ...
    _text_color_rules : List[py_tui.ColorRule]
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
                 padx: int, pady: int, logger, selectable: bool = True):
        ...
        if grid is None:
            raise py_tui.errors.PyCUIMissingParentError("Cannot add widget to NoneType")

        self._grid = grid
        grid_rows, grid_cols = self._grid.get_dimensions()
        if (grid_cols < column + column_span) or (grid_rows < row + row_span):
            raise py_tui.errors.PyCUIOutOfBoundsError(f"Target grid too small for widget {title}")
        ...
        self.key_commands: Dict[int, Callable[[], Any]] = {}
        self._mouse_commands: Dict[int, Callable[[], Any]] = {}
        self._text_color_rules: List['py_tui.ColorRule'] = []
        self._default_color = Colors.default_widget
    ...
    def add_key_command(self, key: Union[int, List[int]], command: Callable[[], Any]) -> None:
        ...
        if isinstance(key, list):
            for value in key:
                self.key_commands[value] = command
        else:
    ...
    def add_mouse_command(self, mouse_event: int, command: Callable[[], Any]) -> None:
        ...
        if mouse_event not in py_tui.keys.MOUSE_EVENTS:
            raise py_tui.errors.PyCUIError(f'Event code {mouse_event} is not a valid py_tui mouse event!')
    ...
    def update_key_command(self, key: Union[int, List[int]], command: Callable[[], Any]) -> Any:
        ...
        if key in self.key_commands.keys():
    ...
    def add_text_color_rule(self, regex: str, color: int, rule_type: str, match_type: str = 'line',
        ...
        color : int
            a supported py_tui color value
        rule_type : string
        ...
        new_color_rule = py_tui.colors.ColorRule(regex, color, selected, rule_type, match_type, region,
                                                 include_whitespace, self._logger)
    ...
    def get_absolute_start_pos(self) -> Tuple[int, int]:
        ...
        if self.column > offset_x:
            x_adjust = offset_x
        if self.row > offset_y:
            y_adjust = offset_y
        x_pos = self.column * col_width + x_adjust
        # noinspection PyProtectedMember
        y_pos = self.row * row_height + y_adjust + self._grid._title_bar_offset + 1
        return x_pos, y_pos
    ...
    def get_absolute_stop_pos(self) -> Tuple[int, int]:
        ...
        counter = self.row
        while counter < offset_y and (counter - self.row) < self._row_span:
            height = height + 1
            counter = counter + 1

        counter = self.column
        while counter < offset_x and (counter - self.column) < self._column_span:
            width = width + 1
            counter = counter + 1
    ...
    def _is_row_col_inside(self, row: int, col: int) -> bool:
        ...
        is_within_rows = self.row <= row <= (self.row + self._row_span - 1)
        is_within_cols = self.column <= col <= (self._column_span + self.column - 1)
    ...
    def get_border_after_draw(self):
        """Must be implemented by subclasses. get border of widget for mouse click events
                """
...
class Label(Widget):
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
                 padx: int, pady: int, logger, override: bool = False):
    ...
    def get_border_after_draw(self):
        _, width = self.get_absolute_dimensions()
        self._border_x_start, self._border_y_start = self._start_x + self._padx, self._start_y + int(self._height/2)
        self._border_height, self._border_width = 2, (width - 2 * self._padx)
    ...
    def _draw(self) -> None:
        ...
        target_y = self._start_y + int(self._height / 2) + self._pady
        self._renderer.draw_text(self, self._title, target_y, centered=True, bordered=self._draw_border, override=self._override)
...
class BlockLabel(Widget):
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
                 padx: int, pady: int, logger, center: bool):
    ...
    def _draw(self) -> None:
        ...
        if self._draw_border:
            self._renderer.draw_border(self, with_title=False)
        counter = self._start_y + 1
        for line in self._lines:
            if counter == self._start_y + self._height - self._pady:
                break
            self._renderer.draw_text(self, line, counter, centered=self._center, bordered=self._draw_border, override=self._override)
            counter = counter + 1
        self._renderer.unset_color_mode(self._color)


class ScrollMenu(Widget, py_tui.ui.MenuImplementation):
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
        ...
        py_tui.ui.MenuImplementation.__init__(self, logger)
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event: int):
        ...
        if mouse_event == py_tui.keys.LEFT_MOUSE_CLICK or mouse_event == py_tui.keys.LEFT_MOUSE_DBL_CLICK:
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if key_pressed == py_tui.keys.KEY_UP_ARROW:
            self._scroll_up()
        if key_pressed == py_tui.keys.KEY_DOWN_ARROW:
            self._scroll_down(viewport_height)
        if key_pressed == py_tui.keys.KEY_HOME:
            self._jump_to_top()
        if key_pressed == py_tui.keys.KEY_END:
            self._jump_to_bottom(viewport_height)
        if key_pressed == py_tui.keys.KEY_PAGE_UP:
            self._jump_up()
        if key_pressed == py_tui.keys.KEY_PAGE_DOWN:
...
class CheckBoxMenu(Widget, py_tui.ui.CheckBoxMenuImplementation):
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
        ...
        Widget.__init__(self, id, title, grid, row, column, row_span, column_span, padx, pady, logger)
        py_tui.ui.CheckBoxMenuImplementation.__init__(self, logger, checked_char)
        self.set_help_text(
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if key_pressed == py_tui.keys.KEY_PAGE_DOWN:
            self._jump_down(viewport_height)
        if key_pressed == py_tui.keys.KEY_ENTER:
...
class Button(Widget):
    ...
    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
        ...
        if self.command is not None:
            self.add_mouse_command(py_tui.keys.LEFT_MOUSE_CLICK, self.command)
            self.add_mouse_command(py_tui.keys.LEFT_MOUSE_DBL_CLICK, self.command)
        self.get_border_after_draw()

    def get_border_after_draw(self):
        _, width = self.get_absolute_dimensions()
        self._border_x_start, self._border_y_start = self._start_x + self._padx, self._start_y + 1
        self._border_height, self._border_width = 2, (width - 2 * self._padx)
    ...
    def _draw(self) -> None:
        ...
        self._renderer.draw_border(self, fill=False, with_title=False, Wtype="button")
        self._renderer.draw_text(self, self._title, self._start_y+2, centered=True, selected=self._selected)
        self._renderer.reset_cursor(self)
        self._renderer.unset_color_mode(self.get_color())


class TextBox(Widget, py_tui.ui.TextBoxImplementation):
    """Widget for entering small single lines of text
    """

    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
                 padx: int, pady: int, logger, initial_text: str, password: bool):
        """Initializer for TextBox widget. Uses TextBoxImplementation as base
        """

        Widget.__init__(self, id, title, grid, row, column, row_span, column_span, padx, pady, logger)
        py_tui.ui.TextBoxImplementation.__init__(self, initial_text, password, logger)
        self.update_height_width()
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event: int) -> None:
        ...
        Widget._handle_mouse_press(self, x, y, mouse_event)
        if y == self._cursor_y and self._cursor_max_left <= x <= self._cursor_max_right:
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        Widget._handle_key_press(self, key_pressed)
        if key_pressed == py_tui.keys.KEY_LEFT_ARROW:
            self._move_left()
        elif key_pressed == py_tui.keys.KEY_RIGHT_ARROW:
            self._move_right()
        elif key_pressed in py_tui.keys.KEY_BACKSPACE:
            self._erase_char()
        elif key_pressed == py_tui.keys.KEY_DELETE:
            self._delete_char()
        elif key_pressed == py_tui.keys.KEY_HOME:
            self._jump_to_start()
        elif key_pressed == py_tui.keys.KEY_END:
            self._jump_to_end()
        elif 31 < key_pressed < 128 or \
                1000 < key_pressed < 1128:
    ...
    def _draw(self) -> None:
        ...
        if self._selected:
            ...
            self._renderer.reset_cursor(self, fill=True)
        self._renderer.unset_color_mode(self._color)


class ScrollTextBlock(Widget, py_tui.ui.TextBlockImplementation):
    """Widget for editing large multi-line blocks of text
    """

    def __init__(self, id, title: str, grid: 'py_tui.grid.Grid', row: int, column: int, row_span: int, column_span: int,
                 padx: int, pady: int, logger, initial_text: str):
        """Initializer for TextBlock Widget. Uses TextBlockImplementation as base
        """

        Widget.__init__(self, id, title, grid, row, column, row_span, column_span, padx, pady, logger)
        py_tui.ui.TextBlockImplementation.__init__(self, initial_text, logger)
        self.update_height_width()
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event: int) -> None:
        ...
        if mouse_event == py_tui.keys.LEFT_MOUSE_CLICK:
            if self._cursor_max_up <= y <= self._cursor_max_down:
                if self._cursor_max_left <= x <= self._cursor_max_right:
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if key_pressed == py_tui.keys.KEY_LEFT_ARROW:
            ...
            self._move_right()
        elif key_pressed == py_tui.keys.KEY_UP_ARROW:
            self._move_up()
        # TODO: Fix this janky operation here
        elif key_pressed == py_tui.keys.KEY_DOWN_ARROW and self._cursor_text_pos_y < len(self._text_lines) - 1:
            self._move_down()
        elif key_pressed in py_tui.keys.KEY_BACKSPACE:
            self._handle_backspace()
        elif key_pressed == py_tui.keys.KEY_DELETE:
            self._handle_delete()
        elif key_pressed == py_tui.keys.KEY_ENTER:
            self._handle_newline()
        elif key_pressed == py_tui.keys.KEY_TAB:
            for _ in range(0, 4):
                self._insert_char(py_tui.keys.KEY_SPACE)
        elif key_pressed == py_tui.keys.KEY_HOME:
            self._handle_home()
        elif key_pressed == py_tui.keys.KEY_END:
            self._handle_end()
        elif 31 < key_pressed < 128:
            self._insert_char(key_pressed)
    ...
    def _draw(self) -> None:
        ...
        for line_counter in range(self._viewport_y_start, self._viewport_y_start + self._viewport_height):
            ...
            self._renderer.draw_text(self, render_text, counter, start_pos=self._viewport_x_start,
