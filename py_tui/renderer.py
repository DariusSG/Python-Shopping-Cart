"""Module containing the py_tui renderer. It is used to draw all of the onscreen ui_elements and items.
"""
...
class Renderer:
    ...
    color_rules : list of py_tui.colors.ColorRule
        List of currently loaded rules to apply during drawing
    """

    def __init__(self, root: 'py_tui.PyCUI', stdscr, logger):
        """Constructor for renderer object
        """

        self._root = root
        self._stdscr = stdscr
        self._color_rules: List['py_tui.colors.ColorRule'] = []
        self._logger = logger
    ...
    def set_color_rules(self, color_rules) -> None:
        ...
        Parameters
        ----------
        color_rules : List[py_tui.colors.ColorRule]
            List of currently loaded rules to apply during drawing
        """
    ...
    def reset_cursor(self, ui_element: 'py_tui.ui.UIElement', fill: bool = True) -> None:
        """Positions the cursor at the bottom right of the selected element

        Parameters
        ----------
        ui_element : py_tui.ui.UIElement
    ...
    def draw_border(self, ui_element: 'py_tui.ui.UIElement', fill: bool = True, with_title: bool = True,
                    Wtype: str = None) -> None:
        ...
        if Wtype == "button":
            border_y_start = start_y + 1
            border_y_stop = border_y_start + 2

        self.set_color_mode(ui_element.get_border_color())

        self._stdscr.addstr(border_y_start, start_x + padx, self._string_border_top(
            (
                self._border_characters["UP_LEFT"], self._border_characters["HORIZONTAL"],
                self._border_characters["UP_RIGHT"]),
            padx, width, title, with_title))

        for i in range(border_y_start + 1, border_y_stop):
            self._stdscr.addstr(i, start_x + padx, self._string_blank_row(self._border_characters["VERTICAL"], padx, width))

        self._stdscr.addstr(border_y_stop, start_x + padx, self._string_border_bottom(
            (
                self._border_characters["DOWN_LEFT"], self._border_characters["HORIZONTAL"],
    ...
    def _string_border_top(border_char: Tuple[str, str, str], padx: int, width: int, title: str, with_title: bool) -> str:
        border_char = [*border_char]
        if not with_title or (len(title) + 4 >= width - 2 * padx):
            return f'{border_char[0]}{border_char[1] * (width - 2 - 2 * padx)}{border_char[2]}'
        else:
            return f'{border_char[0]}{2 * border_char[1]} {title} {border_char[1] * (width - 6 - 2 * padx - len(title))}{border_char[2]}'

    @staticmethod
    @functools.lru_cache(maxsize=256)
    def _string_border_bottom(border_char: Tuple[str, str, str], padx: int, width: int) -> str:
        border_char = [*border_char]
        return f'{border_char[0]}{border_char[1] * (width - 2 - 2 * padx)}{border_char[2]}'

    @staticmethod
    @functools.lru_cache(maxsize=256)
    def _string_blank_row(border_char: str, padx: int, width: int) -> str:
        return f'{border_char}{" " * (width - 2 - 2 * padx)}{border_char}'

    @staticmethod
    def _string_render_text(padx: int, width: int, line: str, centered: bool, bordered: bool, start_pos: int,
                            override: bool = False) -> str:

        render_text_length = width - (2 * padx) - 4 if bordered else width - (2 * padx)
    ...
    def _draw_border_top(self, ui_element: 'py_tui.ui.UIElement', y: int, with_title: bool) -> None:
        ...
        self._stdscr.addstr(y, start_x + padx, self._string_border_top(
            (
                self._border_characters["UP_LEFT"], self._border_characters["HORIZONTAL"],
                self._border_characters["UP_RIGHT"]),
            padx, width, title, with_title))

    def _draw_border_bottom(self, ui_element: 'py_tui.ui.UIElement', y: int) -> None:
        ...
        self._stdscr.addstr(y, start_x + padx, self._string_border_bottom(
            (
                self._border_characters["DOWN_LEFT"], self._border_characters["HORIZONTAL"],
                self._border_characters["DOWN_RIGHT"]),
            padx, width))

    def _draw_blank_row(self, ui_element: 'py_tui.ui.UIElement', y: int) -> None:
        ...
        self._stdscr.addstr(y, start_x + padx, self._string_blank_row(self._border_characters["VERTICAL"], padx, width))

    def _get_render_text(self, ui_element: 'py_tui.ui.UIElement', line: str, centered: bool, bordered: bool, selected,
                         start_pos: int, override: bool = False) -> List[List[Union[int, str]]]:
        ...
        render_text = self._string_render_text(padx, width, line, centered, bordered, start_pos, override)
    ...
    def _generate_text_color_fragments(self, ui_element: 'py_tui.ui.UIElement', line: str, render_text: str,
                                       selected) -> List[List[Union[int, str]]]:
    ...
    def draw_text(self, ui_element: 'py_tui.ui.UIElement', line: str, y: int, centered: bool = False,
                  bordered: bool = True, selected: bool = False, start_pos: int = 0, override: bool = False):
        ...
        render_text = self._get_render_text(ui_element, line, centered, bordered, selected, start_pos, override)
        ...
        for text_elem in render_text:
            if isinstance(text_elem[0], str) and isinstance(text_elem[1], int):
                ...
                if selected and text_elem[1] != py_tui.BLACK_ON_WHITE:
                    self._set_bold()
