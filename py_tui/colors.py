...
_COLOR_MAP = {
    ...
    DEFAULT_COLOR: (curses.COLOR_WHITE, -1),
    DEFAULT_BUTTON_COLOR: (curses.COLOR_MAGENTA, -1),
}

USER_COLOR_PAIRS = {}


def get_colour_pair(fg, bg, initialized: bool = False):
    """Gets the colour pair for a set fg, bg value
        ...
        initialized: bool
            flag to indicate if the program has initialized
        """
    key = (fg, bg)
    if key not in USER_COLOR_PAIRS:
        # Use the pairs from 101 and after, so there's less chance they'll be overwritten by the user
        pair_num = len(USER_COLOR_PAIRS) + 101
        USER_COLOR_PAIRS[key] = pair_num
        if initialized:
            curses.init_pair(pair_num,fg,bg)
    return USER_COLOR_PAIRS[key]
...
class ColorRule:
    ...
    color : int
        A valid color value. Ex. py_tui.WHITE_ON_BLACK
    rule_type : str
    ...
    def __init__(self, regex: str, color: int, selected_color: int, rule_type: str, match_type: str, region: List[int],
                 include_whitespace: bool, logger):
    ...
    def _generate_fragments_regex(self, widget: Union['py_tui.widgets.Widget', 'py_tui.ui.UIElement'], render_text: str,
                                  selected) -> List[List[Union[int, str]]]:
        """Splits text into fragments based on regular expression
        
        Parameters
        ----------
        widget : py_tui.Widget
        ...
        for match in matches:
            ...
            if len(temp) == 2:
                if self._rule_type == 'contains':
                    line_match = match[1:-1]
                else:
                    line_match = match
                if selected:
                    fragments.append([temp[0], widget.get_selected_color()])
                    fragments.append([line_match, self._selected_color])
                else:
                    fragments.append([temp[0], widget.get_color()])
                    fragments.append([line_match, self._color])
    ...
    def _split_text_on_region(self, widget: Union['py_tui.widgets.Widget', 'py_tui.ui.UIElement'], render_text: str,
                              selected) -> List[
        List[Union[str, int]]]:  # renderer._generate_text_color_fragments passes a uielement and not a widget
    ...
    def generate_fragments(self, widget: Union['py_tui.widgets.Widget', 'py_tui.ui.UIElement'], line: str,
                           render_text: str, selected=False) -> Tuple[List[List[Union[str, int]]], bool]:
        # noinspection GrazieInspection
