...
class UIElement:
    ...
    _renderer : py_tui.renderer.Renderer
        The default ui renderer
    _logger   : py_tui.debug.PyCUILogger
    ...
    def __init__(self, id, title, renderer, logger):
        ...
        self._color = py_tui.DEFAULT_COLOR
        ...
        self._logger = logger
        self._help_text = ''
        self._border_x_start, self._border_y_start = 0, 0
        self._border_height, self._border_width = 0, 0
    ...
    def get_renderer(self) -> 'py_tui.renderer.Renderer':
        """Gets reference to renderer object

        Returns
        -------
        renderer : py_tui.renderer.Render
    ...
    def _assign_renderer(self, renderer: 'py_tui.renderer.Renderer', quiet: bool = False):
        ...
        Parameters
        ----------
        renderer : py_tui.renderer.Renderer
        ...
        if renderer is None:
            ...
            raise py_tui.errors.PyCUIError('Renderer already assigned for the element')
        elif isinstance(renderer, py_tui.renderer.Renderer):
            self._renderer = renderer
        else:
            raise py_tui.errors.PyCUIError('Invalid renderer, must be of type py_tui.renderer.Renderer')
    ...
    def _contains_position(self, x: int, y: int) -> bool:
        ...
        """
        within_x = self._border_x_start <= x <= self._border_x_start + self._border_width
        within_y = self._border_y_start <= y <= self._border_y_start + self._border_height
...
class TextBlockImplementation(UIImplementation):
    ...
    def _handle_backspace(self) -> None:
        ...
        if self._cursor_text_pos_x == 0 and self._cursor_text_pos_y != 0:
            ...
            self._text_lines[self._cursor_text_pos_y - 1] = self._text_lines[self._cursor_text_pos_y - 1] + \
                                                            self._text_lines[self._cursor_text_pos_y]
            self._text_lines = self._text_lines[:self._cursor_text_pos_y] + self._text_lines[
                                                                            self._cursor_text_pos_y + 1:]
            self._cursor_text_pos_y = self._cursor_text_pos_y - 1
    ...
    def _handle_delete(self) -> None:
        ...
        if self._cursor_text_pos_x == len(current_line) and self._cursor_text_pos_y < len(self._text_lines) - 1:
            self._text_lines[self._cursor_text_pos_y] = self._text_lines[self._cursor_text_pos_y] + self._text_lines[
                self._cursor_text_pos_y + 1]
            self._text_lines = self._text_lines[:self._cursor_text_pos_y + 1] + self._text_lines[
                                                                                self._cursor_text_pos_y + 2:]
    ...
    def _insert_char(self, key_pressed: int) -> None:
        ...
        self.set_text_line(
            current_line[:self._cursor_text_pos_x] + chr(key_pressed) + current_line[self._cursor_text_pos_x:])
        if len(current_line) <= self._viewport_width:
            self._cursor_x = self._cursor_x + 1
        elif self._viewport_x_start + self._viewport_width < len(current_line):
            self._viewport_x_start = self._viewport_x_start + 1
