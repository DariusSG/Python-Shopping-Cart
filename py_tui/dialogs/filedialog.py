...
class FileSelectImplementation(py_tui.ui.MenuImplementation):
    ...
    def __init__(self, initial_loc: str, dialog_type: str, ascii_icons, logger, limit_extensions: List[str] = [],
        ...
        self.current_dir = os.path.abspath(initial_loc)
    ...
    def refresh_view(self) -> None:
        ...
        if not os.path.exists(self.current_dir):
            ...
            for item in os.listdir(self.current_dir):
                if not self._show_hidden and is_filepath_hidden(os.path.join(self.current_dir, item)):
                    pass
                else:
            ...
            up_dir = os.path.dirname(self.current_dir)
            if self.current_dir != up_dir:
...
class FileSelectElement(py_tui.ui.UIElement, FileSelectImplementation):
    ...
    def __init__(self, root, initial_dir, dialog_type: str, ascii_icons, title, color, command, renderer, logger,
                 limit_extensions: List[str] = []):
        """Initializer for MenuPopup. Uses MenuImplementation as base
        """

        py_tui.ui.UIElement.__init__(self, 0, os.path.abspath(initial_dir), renderer, logger)
        ...
        self._text_color_rules = [
            py_tui.colors.ColorRule('\U0001f4c1', py_tui.BLUE_ON_BLACK, py_tui.WHITE_ON_BLUE,
                                    'startswith', 'region', [3, 1000], False, logger),
            py_tui.colors.ColorRule('<DIR>', py_tui.BLUE_ON_BLACK, py_tui.WHITE_ON_BLUE,
                                    'startswith', 'region', [6, 1000], False, logger),
            py_tui.colors.ColorRule('     ', py_tui.WHITE_ON_BLACK, py_tui.BLACK_ON_WHITE,
                                    'startswith', 'region', [6, 1000], True, logger),
            py_tui.colors.ColorRule('\U0001f5ce', py_tui.WHITE_ON_BLACK, py_tui.BLACK_ON_WHITE,
                                    'startswith', 'region', [3, 1000], False, logger)
        ]
            ...
            if item is not None:
                if os.path.isdir(item._path):
                    ...
                    try:
                        ...
                        self._parent_dialog.display_warning(f'Permission Error Accessing: {self.current_dir} !')
                        self.current_dir = old_dir
                        self.refresh_view()
                    finally:
...
# noinspection PyProtectedMember
class FileNameInput(py_tui.ui.UIElement, py_tui.ui.TextBoxImplementation):
    ...
    def __init__(self, parent_dialog, title, initial_dir, renderer, logger):
        ...
        py_tui.ui.UIElement.__init__(self, 0, title, renderer, logger)
        py_tui.ui.TextBoxImplementation.__init__(self, title, False, logger)
    ...
    def update_height_width(self) -> None:
        ...
        py_tui.popups.Popup.update_height_width(self)
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if key_pressed == py_tui.keys.KEY_LEFT_ARROW:
            ...
            old_dir = self._parent_dialog._file_dir_select.current_dir
...
# noinspection PyProtectedMember
class FileDialogButton(py_tui.ui.UIElement):
    ...
    def __init__(self, parent_dialog, statusbar_msg, command, button_num, *args):
        ...
        if self.get_title() == 'OK':
            self.set_color(py_tui.GREEN_ON_BLACK)
        else:
            self.set_color(py_tui.RED_ON_BLACK)
        ...
        if self.command is not None:
            if self._button_num == 1:
                if self._parent_dialog._dialog_type == 'saveas':
                    ...
                    res = os.path.join(self._parent_dialog._file_dir_select.current_dir,
                                       self._parent_dialog._filename_input.get())
                elif self._parent_dialog._dialog_type == 'opendir':
                    # Otherwise, return the currently opened directory
                    res = self._parent_dialog._file_dir_select.current_dir
                else:
...
class InternalFileDialogPopup(py_tui.popups.MessagePopup):
...
# noinspection PyProtectedMember
class FileDialogPopup(py_tui.popups.Popup):
    ...
    def __init__(self, root, callback, initial_dir, dialog_type, ascii_icons, limit_extensions, color, renderer,
                 logger):
        ...
        py_tui.popups.Popup.__init__(self, root, title, '', color, renderer, logger)
        ...
        self._file_dir_select = FileSelectElement(self, initial_dir, dialog_type, ascii_icons, title, color, None,
                                                  renderer, logger, limit_extensions=limit_extensions)
        self._submit_button = FileDialogButton(self, title, self._submit, 1, '', 'OK', renderer, logger)
        self._cancel_button = FileDialogButton(self, f'Cancel {title}', self._root.close_popup, 2, '', 'ESC', renderer,
                                               logger)
    ...
    def output_valid(self, output) -> Tuple[bool, Optional[str]]:
        ...
        elif self._dialog_type == 'saveas' and not os.access(self._file_dir_select.current_dir, os.W_OK):
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        if self._internal_popup is None:
            ...
            if key_pressed == py_tui.keys.KEY_TAB:
            ...
            elif key_pressed == py_tui.keys.KEY_ESCAPE:
    ...
    # noinspection GrazieInspection,PyProtectedMember
    def _draw(self) -> None:
