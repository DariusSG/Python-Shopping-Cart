"""Form widget for py_tui. Allows for giving user several fillable text fields in one block
"""
...
class FormField(py_tui.ui.TextBoxImplementation):
...
class FormFieldElement(py_tui.ui.UIElement, FormField):
    ...
    def __init__(self, parent_form, field_index: int, field, init_text: str, passwd: bool, required: bool,
                 renderer: 'py_tui.renderer.Renderer', logger):
        """Initializer for the FormFieldElement class
        """

        self._parent_form = parent_form
        self._field_index = field_index
        py_tui.ui.UIElement.__init__(self, 0, field, renderer, logger)
...
class FormImplementation(py_tui.ui.UIImplementation):
...
class Form(py_tui.widgets.Widget, FormImplementation):
...
class InternalFormPopup(py_tui.popups.MessagePopup):
...
# noinspection PyProtectedMember
class FormPopup(py_tui.popups.Popup, FormImplementation):
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event: int) -> None:
        ...
        py_tui.popups.Popup._handle_mouse_press(self, x, y, mouse_event)
