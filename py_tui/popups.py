"""File containing classes for all popups used by py_tui
"""
...
class Popup(py_tui.ui.UIElement):
    ...
    def __init__(self, root: 'py_tui.PyCUI', title: str, text: str, color: int, renderer: 'py_tui.renderer.Renderer', logger):
        ...
        self._text         = text
        self._selected     = True
        self._close_keys   = [py_tui.keys.KEY_ESCAPE]
...
class MessagePopup(Popup):
    ...
    def __init__(self, root, title, text, color, renderer, logger):
        ...
        self._close_keys = [ py_tui.keys.KEY_ENTER,
                            py_tui.keys.KEY_ESCAPE,
                            py_tui.keys.KEY_SPACE,
                            py_tui.keys.KEY_DELETE] + py_tui.keys.KEY_BACKSPACE
...
class YesNoPopup(Popup):
    ...
    def _handle_key_press(self, key_pressed: int):
        ...
        valid_pressed = False
        if key_pressed == py_tui.keys.KEY_Y_LOWER or key_pressed == py_tui.keys.KEY_Y_UPPER:
            ret_val = True
            valid_pressed = True
        elif key_pressed == py_tui.keys.KEY_N_UPPER or key_pressed == py_tui.keys.KEY_N_LOWER:
            ret_val = False
            valid_pressed = True
...
class TextBoxPopup(Popup, py_tui.ui.TextBoxImplementation):
...
class MenuPopup(Popup, py_tui.ui.MenuImplementation):
    """A scroll menu popup.
    ...
    def __init__(self, root: 'py_tui.PyCUI', items, title, color, command, renderer, logger, run_command_if_none):
    ...
    def _handle_mouse_press(self, x: int, y: int, mouse_event):
        ...
        if mouse_event == py_tui.keys.LEFT_MOUSE_CLICK or mouse_event == py_tui.keys.LEFT_MOUSE_DBL_CLICK:
            ...
            if viewport_top <= y and viewport_top + len(self._view_items) - self._top_view >= y:
                ...
                if mouse_event == py_tui.keys.LEFT_MOUSE_DBL_CLICK:
                    ret_val = self.get()
                    self._root.close_popup()
                    self._command(ret_val)
